...
describe("extractJson", () => {
  describe("pass 1: direct parse", () => {
    ...
    it("parses a JSON string primitive", () => {
      expect(extractJson('"hello"')).toEqual(["hello"]);
    });

    it("parses a JSON number primitive", () => {
      expect(extractJson("42")).toEqual([42]);
    });

    it("parses a JSON boolean", () => {
      expect(extractJson("true")).toEqual([true]);
    });

    it("parses null", () => {
      expect(extractJson("null")).toEqual([null]);
    });
  ...
  describe("pass 2: code blocks", () => {
    ...
    it("extracts multiple JSON objects from code blocks", () => {
      const text = '```json\n{"a":1}\n```\nand\n```json\n{"b":2}\n```';
  ...
  describe("pass 3: balanced extraction", () => {
    ...
    it("returns both object and array when both present", () => {
      const text = 'Got [1,2] and {"a":1} here.';
      expect(extractJson(text)).toEqual([{ a: 1 }, [1, 2]]);
    });
  ...
  describe("sentinel", () => {
    it("returns empty array when sentinel is found", () => {
      const text = "NO_FINDINGS: the scan completed with no issues.";
      expect(extractJson(text, "NO_FINDINGS")).toEqual([]);
    });

    it("extracts normally when sentinel is absent", () => {
      const text = '{"result": "found"}';
      expect(extractJson(text, "NO_FINDINGS")).toEqual([{ result: "found" }]);
    });

    it("sentinel takes priority over valid JSON", () => {
      const text = 'NO_FINDINGS {"a":1}';
      expect(extractJson(text, "NO_FINDINGS")).toEqual([]);
    });

    it("ignores sentinel when not provided", () => {
      const text = 'NO_FINDINGS {"a":1}';
      expect(extractJson(text)).toEqual([{ a: 1 }]);
    });
  });
  ...
  describe("failure cases", () => {
    ...
    it("returns empty array for invalid JSON in code block", () => {
      const text = "```json\n{invalid}\n```";
      expect(extractJson(text)).toEqual([]);
    });

    it("returns empty array for unbalanced braces", () => {
      expect(extractJson("{ missing close")).toEqual([]);
    });
