...
describe("Document", () => {
  ...
  describe("chainable methods", () => {
    ...
    it("image() adds an image block and returns this", () => {
      const document = new Document().image(
  ...
  describe("keyValue()", () => {
    ...
    it("filters out undefined values", () => {
      const document = new Document().keyValue({
        Name: "Bob",
        Email: undefined,
      });
      const blocks = document.getBlocks();
  ...
  describe("truncatedList()", () => {
    ...
    it('truncates with "and X more" message', () => {
      const document = new Document().truncatedList(["a", "b", "c", "d", "e"], {
        limit: 3,
      });
      expect(document.getBlocks()[0]).toEqual({
        type: "text",
    ...
    it("uses default limit of 10", () => {
      ...
      const content = (
        document.getBlocks()[0] as { type: "text"; content: string }
      ).content;
