...
describe("buildFileTree", () => {
  describe("markdown format (default)", () => {
    it("wraps output in a code fence by default", () => {
      const result = buildFileTree(["src/index.ts", "README.md"]);
      expect(result).toBe("```\nsrc/\n  index.ts\n\nREADME.md\n```");
    });
  ...
  describe("plain format", () => {
    ...
    it("truncates level 2 children", () => {
      ...
      const result = buildFileTree(paths, { maxLevel2: 3, format: "plain" });
      const lines = result.split("\n");
      expect(lines).toContain("  .. (12 more)");
    });

    it("returns empty string for no paths", () => {
      expect(buildFileTree([], { format: "plain" })).toBe("");
    });
    ...
    describe("collapseDirs", () => {
      it("collapses matching directory with summary", () => {
        const result = buildFileTree(
          ...
          { collapseDirs: ["test"], format: "plain" }
        );
        expect(result).toBe(
