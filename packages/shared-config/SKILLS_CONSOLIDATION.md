...
**Package-level**: Edit `external-skills.txt` in the package:

```
# Add new repos (owner/repo format)
new-org/agent-skills
```

**Project-level**: Create `external-skills.txt` in your project root:
...
Then run `npm run sync-external-skills` to fetch. Duplicates are safely handled.
