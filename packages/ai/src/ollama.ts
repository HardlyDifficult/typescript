...
const ollamaAgent = new Agent({
  headersTimeout: duration({ hours: 1 }), // 60 min — allows for large model loading
  bodyTimeout: duration({ minutes: 30 }), // 30 min — streaming chunks should arrive regularly once started
  connectTimeout: duration({ minutes: 1 }), // 1 min  — localhost connection should be near-instant
});
